...
def add_user():
    ...
    if "username" not in request_body:
        raise APIException('The username is required', 400)
    ...
    exists_username = User.query.filter_by(username = request_body['username']).first()
...
def login():
    ...
    if user is None:
        raise APIException ('The email is not correct', status_code=401)
...
@jwt_required() #ligado con el autorization
def protected():
